...
def build(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    """
    context.run(
        f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} build --build-arg netbox_ver={netbox_ver} --build-arg python_ver={python_ver} --no-cache",
...
@task
def createuser(context, user="admin", netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
...
def makemigrations(context, name="", netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    if name:
        ...
        context.run(
            f"docker-compose -f {COMPOSE_FILE} -p {BUILD_NAME} run netbox python manage.py makemigrations aisPlugin",
            env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        )
